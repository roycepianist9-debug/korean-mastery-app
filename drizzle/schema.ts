 ...
 */
export const customWordLists = mysqlTable("custom_word_lists", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  language: mysqlEnum("language", ["korean", "chinese", "japanese", "english"]).notNull().default("korean"),
  color: varchar("color", { length: 7 }).default("#10b981"), // hex color for UI
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => [
  index("idx_custom_lists_user").on(table.userId),
  index("idx_custom_lists_language").on(table.language),
]);

export type CustomWordList = typeof customWordLists.$inferSelect;
export type InsertCustomWordList = typeof customWordLists.$inferInsert;

/**
 * Words in custom lists (junction table)
 */
export const customListWords = mysqlTable("custom_list_words", {
  id: int("id").autoincrement().primaryKey(),
  listId: int("listId").notNull().references(() => customWordLists.id, { onDelete: "cascade" }),
  wordId: int("wordId").notNull().references(() => words.id, { onDelete: "cascade" }),
  addedAt: timestamp("addedAt").defaultNow().notNull(),
}, (table) => [
  index("idx_list_words_list").on(table.listId),
  index("idx_list_words_word").on(table.wordId),
  index("idx_list_words_unique").on(table.listId, table.wordId),
]);

export type CustomListWord = typeof customListWords.$inferSelect;
export type InsertCustomListWord = typeof customListWords.$inferInsert;
