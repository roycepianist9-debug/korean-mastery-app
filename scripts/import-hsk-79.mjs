...
async function generateMeaningAndExample(wordBatch) {
...
async function translateExamples(examples) {
...
async function main() {
  ...
  for (let i = 0; i < allExamples.length; i += TRANSLATION_BATCH) {
    ...
    try {
      ...
      console.error(`✗ Error in translation batch ${batchNum}: ${error.message}\n`);
